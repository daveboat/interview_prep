...
class Solution(object):
    def compareVersion(self, version1, version2):
        ...
        :rtype: int
        """
        # so split, convert to int (this implicitly gets rid of leading zeroes), then do a compare, treating
        # the leftmost digit as the most significant digit

        v1 = [int(i) for i in version1.split('.')]
        v2 = [int(i) for i in version2.split('.')]

        # for convenience, add zeroes to the end of the shorter version number
        if len(v1) > len(v2):
            v2 += [0] * (len(v1) - len(v2))
        elif len(v2) > len(v1):
            v1 += [0] * (len(v2) - len(v1))
